...
Timestamp EpollPoller::waitPoll(int timeout,
  ...
  if (nfds > 0) {
    // Traverse the ready events, update revents of the corresponding Channel, insert these actived channel into the array
...
void EpollPoller::EpollCtlOperate(Channel* channel, int operation) {
  ...
  if (ret < 0) {
    if (operation == EPOLL_CTL_ADD) {
      LOG_ERROR(
          "epoll Ctl Operation EPOLL_CTL_ADD error : %d || Channal State : %d "
          "/ fd : %d / Event : %d / REvent : %d",
          errno, channel->getCurState(), channel->getFd(), channel->getEvents(),
          channel->getRevents());
